...
+ (BOOL)contactExist: (ABRecordID) recordID {
	ABRecordRef contactRec = ABAddressBookGetPersonWithRecordID(addressBook, recordID);
	if (contactRec) {
		return YES;
	}
...
+ (BOOL)contacthasChangedSinceLastTime: (NSDate *) date InRecordID: (ABRecordID) recordID {
	ABRecordRef contactRec = ABAddressBookGetPersonWithRecordID(addressBook, recordID);
	if (contactRec) {
...
- (NSString *) contactName
{
	...
	NSString *cFirstName = nil;
	NSString *cLastName = nil;
	
	cFirstName = self.firstname;
	cLastName = self.lastname;
	...
	if (cFirstName || cLastName)
	{
		...
		if (cFirstName) [string appendFormat:@"%@", cFirstName];
