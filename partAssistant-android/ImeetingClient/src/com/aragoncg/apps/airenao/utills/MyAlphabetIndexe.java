...
public class MyAlphabetIndexe extends AlphabetIndexer {
	 ...
	 * whenever the dataset changes or the cursor changes.
	 */
	...
	public int getPositionForSection(int sectionIndex) {
		...
		while (pos < end) {
			...
			if (!AirenaoUtills.isStartEnChar(curName)) {
				...
				curName = AirenaoUtills.getPinYin(String.valueOf(curName
						.charAt(0)));
			}
			if (curName == null) {
				if (pos == 0) {
			...
			if (diff != 0) {
				// TODO: Commenting out approximation code because it doesn't
				// work for certain
