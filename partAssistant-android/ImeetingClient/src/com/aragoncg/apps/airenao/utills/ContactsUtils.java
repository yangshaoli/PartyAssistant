...
public class ContactsUtils {
	...
	public static final CharSequence getDisplayLabel(Context context,
			String mimetype, int type, CharSequence label) {
	 ...
	 * {@link android.provider.Im.ProviderNames} from the predefined IM protocol
	 * id. This is used for interacting with the IM application.
	 * 
	 * @param protocol
	 *            the protocol ID
	 * @return the provider name the IM app uses for the given protocol, or null
	 *         if no provider is defined for the given protocol
	 * @hide
	 */
	...
	public static Intent buildImIntent(ContentValues values) {
		final boolean isEmail = Email.CONTENT_ITEM_TYPE.equals(values
		...
		final int protocol = isEmail ? Im.PROTOCOL_GOOGLE_TALK : values
				.getAsInteger(Im.PROTOCOL);
		...
		if (!TextUtils.isEmpty(host) && !TextUtils.isEmpty(data)) {
			...
			final Uri imUri = new Uri.Builder().scheme(Constants.SCHEME_IMTO)
					.authority(authority).appendPath(data).build();
	...
	public static String querySuperPrimaryPhone(ContentResolver cr,
		...
		try {
			...
			c = cr.query(dataUri, new String[] { Phone.NUMBER }, Data.MIMETYPE
					+ "=" + Phone.MIMETYPE + " AND " + Data.IS_SUPER_PRIMARY
					+ "=1", null, null);
	...
	public static ArrayList<Long> queryForAllRawContactIds(ContentResolver cr,
			long contactId) {
		Cursor rawContactIdCursor = null;
	 ...
	 * 
	 * @param parent
	 *            The parent ViewGroup to attach the new view to.
	 * @param label
	 *            The label to display in the tab indicator. If null, not label
	 *            will be displayed.
	 * @param icon
	 *            The icon to display. If null, no icon will be displayed.
	 * @return The tab indicator View.
	 */
	public static View createTabIndicatorView(ViewGroup parent,
	...
	/**
	 * 
	 * Method:GetNumber:(......) TODO(to revert the 11 phone number)
	 * 
	 * @author cuikuangye
	 * @param num
	 * @return String
